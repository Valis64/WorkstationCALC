...
from time_utils import business_hours_breakdown, hours_breakdown
...
def compute_lead_times(rows, start_date=None, end_date=None, show_breakdown=False):
    ...
    for row in rows:
        ...
        business_delta, after_hours_delta = hours_breakdown(
            row["time_in"], row["time_out"]
        )
        hours_in_business = business_delta.total_seconds() / 3600.0
        hours_after_hours = after_hours_delta.total_seconds() / 3600.0
            ...
            breakdowns[row["job_number"]].append(
                {
                    ...
                    "segments": segments,
                    "after_hours": hours_after_hours,
                }
            )

        results[row["job_number"]].append(
            {
                "workstation": row["workstation"],
                "hours_in_business": hours_in_business,
                "hours_after_hours": hours_after_hours,
            }
        )
...
def format_breakdown(job_number, workstation_name, segments, after_hours):
    ...
    lines.append(f"  After hours: {after_hours:.2f}h")
    return "\n".join(lines)
                ...
                writer.writerow(
                    {
                        ...
                        "hours_in_business": f"{step['hours_in_business']:.2f}",
                        "after_hours": f"{step['hours_after_hours']:.2f}",
                    }
                )
