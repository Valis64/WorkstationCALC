...
from time_utils import calculate_hours
...
def parse_manage_html(path):
    ...
    for tr in soup.select("tbody#table tr"):
        ...
        if not match:
            logger.warning("Could not find job ID in row: %s", job_text)
            continue
...
def compute_lead_times(jobs, start_date=None, end_date=None):
    ...
    and the end timestamp is on or before ``end_date``. Hours are computed via
    :func:`calculate_hours`, which accepts ``datetime`` objects or ISO-formatted
    strings and counts only configured business hours.
    """
    ...
    for job, steps in jobs.items():
        for (name, start), (next_name, end) in zip(steps, steps[1:]):
            ...
            if end_date and end > end_date:
                continue
            # ``calculate_hours`` handles datetimes directly and accounts for
...
def generate_realtime_report(jobs, start_date=None, end_date=None):
    ...
    is the total number of hours between them.
    """
